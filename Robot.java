...
public class Robot extends TimedRobot implements RobotProperties {
  ...
  private PhotonAprilTagTarget bestGeneralTargetVC;
  private PhotonAprilTagTarget bestReefTargetVC;
  ...
  public void robotInit() {
    ...
    robotToCam = new Transform3d( // Depends on driver station, come back to this later
      new Translation3d(5 * 0.0254, // X: Forward/Backward from center (+/-)
                        9 * 0.0254, // Y: Left/Right from center (+/-)
                        32.49 * 0.0254 // Z: Up/Down from center (+/-)
                        ),
      new Rotation3d()
    );
    closeDist = 1; // Initialize at "large" value
  ...
  private void shuffleboardInit() {
    ...
    tagPose3D = fieldLayout.getTagPose(9).get(); // .get() is necessary to retrieve Pose3d from Optional<Pose3d>
    ...
    rearCam.addDouble("aTag Pose X", () -> tagPose2D.getX());
    rearCam.addDouble("aTag Pose Y", () -> tagPose2D.getY());
    rearCam.addDouble("aTag Pose Rot", () -> tagPose2D.getRotation().getDegrees());
    rearCam.addDouble("aGoal Pose X", () -> goalPose2D.getX());
    rearCam.addDouble("aGoal Pose Y", () -> goalPose2D.getY());
    rearCam.addDouble("aGoal Pose Rot", () -> goalPose2D.getRotation().getDegrees());
    gyroPIDController.updateSensorLockValueWithoutReset(-tagPose2D.getRotation().getDegrees());
    seeGyro = gyroPIDController.getSensorLockValue();
    rearCam.addDouble("seeGyro", () -> seeGyro);
  ...
  private void driveControlsPeriodic(final XboxControllerState driveControllerState, final double gyroValue) {
    ...
    if(!targetLockLeft && !targetLockRight) targetLocked = false; // Reset target lock if not auto driving
    ...
    if (rightStickX != 0 || robotOffGround) {
      ...
      switch (DriverStation.getAlliance().get()) {
        case Red:
          bestGeneralTargetVC = visionController.getClosestAprilTagById(RED_FIDUCIAL_IDS, "Arducam_USB_Camera");
          bestReefTargetVC = visionController.getClosestAprilTagById(new int[]{6,7,8,9,10,11}, "Arducam_USB_Camera");
          break;
        default:
          bestGeneralTargetVC = visionController.getClosestAprilTagById(BLUE_FIDUCIAL_IDS, "Arducam_USB_Camera");
          bestReefTargetVC = visionController.getClosestAprilTagById(new int[]{17,18,19,20,21,22}, "Arducam_USB_Camera");
          break;
      }

      // Estimate robot pose using best april tag
      if(bestGeneralTargetVC != null) {
      ...
      if(bestReefTargetVC != null && !targetLocked) {
        int tagID = bestReefTargetVC.getPHOTON_TRACKED_TARGET().getFiducialId();
        tagPose2D = fieldLayout.getTagPose(tagID).get().toPose2d();
        goalPose2D = new Pose2d(
          tagPose2D.getTranslation().plus(new Translation2d(1.0, 0.0).rotateBy(tagPose2D.getRotation())),
        ...
        switch (DriverStation.getAlliance().get()) {
          case Red:
            ...
            break;
          default:
            gyroPIDController.updateSensorLockValueWithoutReset(-goalPose2D.getRotation().getDegrees());
            break;
        }
        targetLocked = true;
      }
      ...
      if(goalPose2D != null) {
        closeDist = Math.abs(robotPose2D.getTranslation().getDistance(goalPose2D.getTranslation()));
        switch (DriverStation.getAlliance().get()) {
          case Red:
            horizontalDiff = goalPose2D.getY() - robotPose2D.getY(); // Field pose -Y corresponds to Controller +X
            verticalDiff = robotPose2D.getX() - goalPose2D.getX(); // Field Pose +X corresponds to Controller +Y
            break;
          default:
        ...
        directDriveAngle = FIELD_ORIENTED_SWERVE ? Normalize_Gryo_Value(directDriveAngle - gyroValue) : directDriveAngle;
        trackingMagnitude = 0.15;
      }
      
      final boolean closeEnough = Math.abs(Get_Gyro_Displacement(gyroValue, gyroPIDController.getSensorLockValue())) <= 1;
      if(closeDist > .3) {
        swerveDrive.drive(directDriveAngle, trackingMagnitude, closeEnough ? 0 : gyroPIDController.getPIDValue(), false);
      } else {
      ...
      gyroPIDController.enablePID();
      final boolean closeEnough = Math.abs(Get_Gyro_Displacement(gyroValue, gyroPIDController.getSensorLockValue())) <= 1;
      swerveDrive.drive(fieldCorrectedAngle,  leftStickMagnitude, closeEnough ? 0 : gyroPIDController.getPIDValue(), false);
    }
